...
class HCCrawler extends EventEmitter {
  ...
  constructor(browser, options) {
    ...
    this._onSkip = options.onSkip || null;
    this._onError = options.onError || null;
    this._onDisallow = options.onDisallow || null;
    this._customCrawl = options.customCrawl || null;
    this._evaluatePage = options.evaluatePage || null;
    this._exposeFunctions = options.exposeFunctions || null;
    this._logger = options.logger || null;
    this._exportHeader();
    this._queue.on('pull', (_options, depth, previousUrl) => this._startRequest(
      {
        ..._options,
        evaluatePage: this._evaluatePage,
        exposeFunctions: this._exposeFunctions,
        logger: this._logger,
      },
      depth,
      previousUrl,
    ...
    this.on(
      HCCrawler.Events.RequestDisallowed,
      async (_options, depth, previousUrl) => {
        if (this._onDisallow) {
          await this._onDisallow(
            extend({}, { options: _options, depth, previousUrl }),
          );
        }
      },
    );
    this.on(
      HCCrawler.Events.RequestSkipped,
      async (_options, type, depth, previousUrl) => {
        if (this._onSkip) {
          const out = {};
          extend(out, {
            options: _options,
          ...
          if (type === HCCrawler.Skip.Requested && _options.skipDuplicates) {
            const result = await this._getContent(_options);
            extend(out, result);
          }

          await this._onSkip(out);
        }
      },
    );
  }
   ...
   */
  async queue(options) {
    const arrOptions = isArray(options) ? options : [options];
    // eslint-disable-next-line arrow-parens
    const promises = map(arrOptions, async (_options) => {
  ...
  async _skipRequest(options) {
    ...
    if (!allowedDomain) return HCCrawler.Skip.AllowedDomains;
    const requested = await this._checkRequested(options);
    if (requested) return HCCrawler.Skip.Requested;
    const shouldRequest = await this._shouldRequest(options);
    if (!shouldRequest) return HCCrawler.Skip.PreRequest;
    return null;
  }
   ...
   * @private
   */
  async _setContent(options, content) {
    if (!options.skipDuplicates) return null;
    const key = generateKey(options);
    return this._cache.set(`${key}:content`, content);
  }
  ...
  async _request(options, depth, previousUrl, retryCount = 0) {
    ...
    try {
      ...
      await this._success(res, options);
      return res.links;
    } catch (error) {
      await crawler.close();
      extend(error, {
   ...
   * @private
   */
  async _success(result, options) {
    if (options.skipDuplicates) {
      await this._setContent(options, result);
    }
  ...
  async _newCrawler(options, depth, previousUrl) {
    ...
    return new Crawler(page, options, depth, previousUrl, this._cache);
  }
  ...
  async _crawl(crawler) {
    ...
    const crawl = (arg1, arg2, arg3) =>
      // eslint-disable-next-line implicit-arrow-linebreak
      crawler.crawl.call(crawler, arg1, arg2, arg3);
