...
class Crawler {
   ...
   */
  constructor(page, options, depth, previousUrl, cache) {
    this._page = page;
   ...
   */
  async crawl(xrequest = true, xcrape = true, xcreenshot = true) {
    let output = {};
    let response;
    ...
    if (xrequest) {
      ...
      output = {
        ...
        links: await this._collectLinks(response.url),
        result: null,
        screenshot: null,
      };
    }
    ...
    if (xcrape) {
      output.result = await this._scrape();
    }

    if (xcreenshot) {
      output.screenshot = await this._screenshot();
    }

    if (xrequest) {
      await this._cache.set(`crawl_${this._options.url}`, output);
    }

    return {
      ...((await this._cache.get(`crawl_${this._options.url}`)) || {}),
      ...output,
    };
  }
  ...
  _handlePageEvents() {
    ...
    this._page.on(
      'console',
      msg => void debugConsole(`${msg.type()} ${msg.text()} at ${this._options.url}`),
    );
  ...
  async _scrape() {
    if (
      ...
      || !Array.isArray(this._options.evaluatePage)
    ) {
      return null;
    }
    ...
    if (
      this._options.exposeFunctions
      && Array.isArray(this._options.exposeFunctions)
      && this._options.exposeFunctions.length > 0
    ) {
      await Promise.all(
        this._options.exposeFunctions.map(value => {
          if (
            value.function
            ...
            return this._page.exposeFunction(
              value.functionName,
              value.function,
            );
          }
    ...
    const out = await Promise.all(
      this._options.evaluatePage.map(value => {
        if (
          value.function
          && typeof value.function === 'function'
          && value.functionArgs
          && Array.isArray(value.functionArgs)
        ) {
          const args = value.functionArgs.reduce((acc, curr) => {
            acc = acc.concat(this._options[curr]);
            return acc;
          }, []);

          return this._page.evaluate(value.function, ...args);
        }
